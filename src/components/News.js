...
export class News extends Component {
  ...
  async componentDidMount() {
    ...
    let url =`https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=fdbc1e5e395b4d31b343b7fe482b6655&page=1&pagesize=${this.props.pageSize}`;
  ...
  handleNext = async ()=>{
    if(!(this.state.page +1 > Math.ceil(this.state.totalResults/9))){
      this.setState({loading: true})
      let url =`https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=fdbc1e5e395b4d31b343b7fe482b6655&page=${this.state.page + 1}&pagesize=${this.props.pageSize}`;
      let data = await fetch(url);
      let parsedData = await data.json();
      this.setState({
